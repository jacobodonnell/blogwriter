---
title: Themes
description: Build and customize themes with file-based routing, Blade templates, and modern web technologies.
...
<x-callout type="info" title="For Developers">
  BlogWriter uses Laravel Folio under the hood. The brackets (`[Article:slug]`) tell Folio to look up the model by that field — no routing code needed.
</x-callout>
...
2. Copy the markup from the [Components](/docs/customization/components) doc
3. Modify it however you want

See the [Components](/docs/customization/components) doc for a complete list of available components with copyable code.
...
- **Check your microformats.** The built-in components include proper microformats markup. If you build custom templates, see the [Feeds & IndieWeb](/docs/configuration/feeds-and-indieweb) doc.
...
#### [Up Next: *Components*](/docs/customization/components)

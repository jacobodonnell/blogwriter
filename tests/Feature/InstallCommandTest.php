...
it('command has correct signature and description', function () {
...
it('has seed option', function () {
    $command = new InstallCommand;

    expect($command->getDefinition()->hasOption('seed'))->toBeTrue();
});

it('detects existing installation', function () {
    User::factory()->create();
...
it('detects no installation when users table empty', function () {
    $command = new InstallCommand;
...
it('can add new env value', function () {
    $command = new InstallCommand;

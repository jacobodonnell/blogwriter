...
describe('reset service integration', function (): void {
    it('verifies ResetService is used in InstallCommand', function (): void {
        // Verify InstallCommand imports and uses ResetService
        $reflection = new \ReflectionClass(\App\Console\Commands\InstallCommand::class);
        $fileContent = file_get_contents($reflection->getFileName());

        // Verify ResetService is imported
        expect($fileContent)->toContain('use App\Services\ResetService;');

        // Verify app(ResetService::class) is called (not Artisan::call('blogwriter:reset'))
        expect($fileContent)->toContain('app(ResetService::class)');
        expect($fileContent)->toContain('$resetService->reset($this)');

        // Verify old Artisan::call to blogwriter:reset is removed
        expect($fileContent)->not->toContain("Artisan::call('blogwriter:reset'");
    });
});

describe('key generation performance', function (): void {
    it('generates key quickly without Artisan call overhead', function (): void {
        ...
        // Key generation should be nearly instant (well under 5 seconds)
        // Previous bug caused 20+ second hang
        expect($duration)->toBeLessThan(5.0);
    });

    it('generates valid base64-encoded 32-byte key', function (): void {
        $this->artisan('blogwriter:install', [
            '--site-name' => 'Test Blog',
            '--site-url' => 'https://test.com',
            '--admin-name' => 'Test User',
        ...
        $envContent = file_get_contents(base_path('.env'));
        preg_match('/APP_KEY=base64:([A-Za-z0-9+\/=]+)/', $envContent, $matches);

        expect($matches)->toHaveCount(2);

        $decodedKey = base64_decode($matches[1]);
        expect(strlen($decodedKey))->toBe(32); // AES-256-CBC requires 32 bytes
    });

    it('generates different keys on each install', function (): void {
        // First install
        $this->artisan('blogwriter:install', [
            '--site-name' => 'Test Blog',
            '--site-url' => 'https://test.com',
        ...
        $envContent1 = file_get_contents(base_path('.env'));
        preg_match('/APP_KEY=(base64:[A-Za-z0-9+\/=]+)/', $envContent1, $matches1);
        $key1 = $matches1[1];

        // Clean up completely for second install
        @unlink(storage_path('installed.lock'));
        \App\Models\User::query()->delete();

        // Second install with --force to skip interactive prompts
        $this->artisan('blogwriter:install', [
            '--site-name' => 'Test Blog',
            '--site-url' => 'https://test.com',
            '--admin-name' => 'Test User',
        ...
        $envContent2 = file_get_contents(base_path('.env'));
        preg_match('/APP_KEY=(base64:[A-Za-z0-9+\/=]+)/', $envContent2, $matches2);
        $key2 = $matches2[1];
